...
In both settings, we essentially leverage a GAN-like architecture, where we train a **Generator** that crafts adversarial patches, using the adversarial loss function, which is then followed by a **Deployer** that simulates random placement of patches.

Generator training is same for both settings, with the only difference in patch sizes. Deployment, on the other hand, is a bit different. We explore these in detail below.
...
### Patch Sizes
- `64x64` patches, that cover about 8% of the original image
- `80x80` patches, that cover about 13% of the original image.
...
In this setting, we train a generator that creates much smaller patches, that are deployed at **several** point in the input image. Note, that the patches are the same across the image, which maintains consistency.

We further differentiate between 3 types of Mini-Patch attacks:

- **Random Placement Patches**: here, we imitate G-Patch implementation, only instead of a single large patch we deploy much smaller patch at several **random** places.
...
- `16x16` patches, which are the main focus of our experiments. These patches are deployed at **8** different places in the image.
- `32x32` patches, which are used for ViT models that use the same dimensions for image tokenization. These patches are deployed at **4** different places in the input image.

Overall, for both patch sizes, adversarial regions are about 8% of the original image, i.e. same coverage as of 64x64 G-Patch.
